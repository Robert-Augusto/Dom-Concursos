...
export default function LessonPage() {
  ...
  useEffect(() => {
    ...
    void RecordLessonWatch(profile.id, activeLesson.id)
  }, [profile?.id, activeLesson?.id])
                  ...
                  <div className="flex items-center gap-1">
                    {isAdmin && moduleId ? (
                      <button
                        type="button"
                        onClick={() => setCreateModalOpen(true)}
                        className="inline-flex h-8 items-center gap-1 rounded-lg border border-border bg-card px-2 text-[10px] font-bold text-foreground transition-colors hover:border-primary/40"
                      >
                ...
                <div className="min-h-0 flex-1 overflow-y-auto">
                  {moduleLessons.length > 0 ? (
                    moduleLessons.map((lesson, index) => {
                      ...
                      const ThumbnailIcon = hasVideoUrl ? Play : FileText
                      const isReordering = reorderingLessonId === lesson.id

                      return (
                        <div
                          key={lesson.id}
                          className={`flex w-full items-center gap-1 border-b border-border/60 last:border-b-0 ${
                            isCurrent
                        ...
                        >
                          ...
                          <button
                            ...
                            className={`flex min-w-0 flex-1 cursor-pointer items-center gap-3 px-2 py-3 text-left transition-colors ${
                          ...
                          >
                            ...
                            <div className="relative flex h-12 w-16 flex-shrink-0 items-center justify-center overflow-hidden rounded-lg bg-muted">
                              <div
                              ...
                              />
                              <ThumbnailIcon className="relative h-4 w-4 text-foreground/70" />
