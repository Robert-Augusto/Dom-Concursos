...
import { CreateSimuladoSessionAnswears } from '@/lib/lib-simulado-session-answears'
...
export default function SimuladoSession({
  ...
  async function handleResolve(questionId: string) {
    ...
    if (!simulado_id) {
      toast.error('Sessão de simulado inválida. Reinicie o simulado.')
      return
    }
    ...
    try {
      const { error } = await CreateSimuladoSessionAnswears(
        String(simulado_id),
        questionId,
        selected,
        isCorrect,
